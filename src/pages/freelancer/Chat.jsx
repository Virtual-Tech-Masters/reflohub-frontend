...
import { FiMessageSquare, FiBriefcase, FiSend, FiWifi, FiWifiOff } from 'react-icons/fi';
import { Helmet } from 'react-helmet-async';
import { useFreelancerChatList } from '../../hooks/useFreelancerChatList';
...
import { renderMessageWithMentions } from '../../components/chat/LeadMention';
...
const FreelancerChat = () => {
  ...
  const handleMentionClick = (leadId) => {
    ...
    navigate(`/freelancer/lead-details/${id}`);
  };

  const { businesses, loading: listLoading, error: listError } = useFreelancerChatList();
  const { 
    messages, 
    ...
    sending, 
    sendMessage 
  } = useFreelancerBusinessChat(null, selectedBusinessId, 'freelancer');

  const selectedBusiness = businesses.find(b => b.id === selectedBusinessId);

  // Check URL params for businessId and leadId on mount and after businesses load
  useEffect(() => {
    const businessIdFromUrl = searchParams.get('businessId');
    const leadIdFromUrl = searchParams.get('leadId');
    const leadNameFromUrl = searchParams.get('leadName');
    
    if (businessIdFromUrl && !listLoading && businesses.length > 0) {
      const id = parseInt(businessIdFromUrl, 10);
      if (!isNaN(id)) {
        // Verify business exists in the list
        const businessExists = businesses.some(b => b.id === id);
        if (businessExists) {
          ...
          toast.error('Business not found or you cannot chat with this business');
        }
        // Clean up URL param
        navigate('/freelancer/chat', { replace: true });
      }
    }
  }, [searchParams, navigate, listLoading, businesses]);
  ...
  return (
    <>
      <Helmet>
        <title>Chat - Freelancer Portal</title>
        <meta name="description" content="Chat with businesses" />
      </Helmet>
      ...
      <PageTitle
            title="Chat"
            subtitle="Communicate with businesses you've submitted leads to"
          />
              ...
              {listLoading ? (
                ...
                <div className="flex-1 flex items-center justify-center p-4">
                  <div className="text-center">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">No businesses to chat with</p>
                    <p className="text-sm text-gray-500 dark:text-gray-500 mt-1">
                      Businesses will appear here after you submit leads
              </p>
            </div>
          </div>
              ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-gray-900 dark:text-white truncate">
                            {escapeHtml(business.name || 'Unknown')}
                          ...
                          {business.category && (
                            <p className="text-sm text-gray-500 dark:text-gray-400 truncate">
                              {escapeHtml(business.category.name || '')}
                            </p>
                          )}
                        </div>
                    </div>
                ...
                <div className="flex-1 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">
                      Choose a business from the list to view your conversation
                    </p>
                  </div>
                </div>
              ) : (
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-blue-100 dark:bg-blue-900/20 rounded-full flex items-center justify-center">
                        ...
                        <h3 className="font-semibold text-gray-900 dark:text-white">
                          {escapeHtml(selectedBusiness?.name || 'Unknown')}
                          {selectedBusiness?.hasVerifiedBadge && (
                            <span className="ml-2 text-blue-500" title="Verified">✓</span>
                          )}
                          </h3>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {messagesLoading ? (
                      ...
                      messages.map((message) => {
                        ...
                        const isCurrentUser = isFreelancer; // Freelancer is always current user for freelancer portal
